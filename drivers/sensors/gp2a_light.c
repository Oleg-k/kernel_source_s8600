...
static const int adc_table[4] = {
	324, //15 lux match adc value
	780, //150 lux match adc value
	1233, //1500 lux match adc value
	1640, //15000 lux match adc value
};


#if 0		//this for function readind ADC
...
static ssize_t
light_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input_data = to_input_dev(dev);
	struct sensor_data *data = input_get_drvdata(input_data);
...
static ssize_t
light_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input_data = to_input_dev(dev);
	struct sensor_data *data = input_get_drvdata(input_data);
...
static ssize_t
light_wake_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct input_dev *input_data = to_input_dev(dev);
	static int cnt = 1;
...
static ssize_t
light_data_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct input_dev *input_data = to_input_dev(dev);
	struct sensor_data *data = input_get_drvdata(input_data);
	//unsigned long flags;
	...
	mutex_lock(&data->mutex);
	light_lux = StateToLux(data->light_level_state);
	mutex_unlock(&data->mutex);	
...
static ssize_t
light_status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	//struct input_dev *input_data = to_input_dev(dev);
	//struct sensor_data *data = input_get_drvdata(input_data);
...
lightsensor_probe(struct platform_device *pdev)
{
	...
	rt = sysfs_create_group(&input_data->dev.kobj, &lightsensor_attribute_group);
	if (rt) 
	{
		printk(KERN_ERR
...
ifdef MSM_LIGHTSENSOR_ADC_READ  
	...
	data->switch_cmd_dev = device_create(data->lightsensor_class, NULL, 0, NULL, "switch_cmd");
	...
	if (device_create_file(data->switch_cmd_dev, &dev_attr_lightsensor_file_state) < 0) 
	{
