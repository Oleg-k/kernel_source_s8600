...
static ssize_t
proximity_delay_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t
proximity_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    struct input_dev *input_data = to_input_dev(dev);
    struct gp2a_data *data = input_get_drvdata(input_data);
...
static ssize_t
proximity_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct input_dev *input_data = to_input_dev(dev);
    struct gp2a_data *data = input_get_drvdata(input_data);
    int enabled;
...
//	}
	...
	gp2a->proximity_dev = device_create(gp2a->proximity_class, NULL, 0, NULL, "proximity");
	...
	if (device_create_file(gp2a->proximity_dev, &dev_attr_proximity_state) < 0) 
	{
	...
	if (device_create_file(gp2a->proximity_dev, &dev_attr_proximity_avg) < 0) 
	{
		pr_err("%s: could not create device file(%s)!\n", __func__,
