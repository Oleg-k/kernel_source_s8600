...
#if defined(CONFIG_MACH_ANCORA) || defined(CONFIG_MACH_S8600) || defined(CONFIG_MACH_HUANGHE)
...
extern int vreg_onoff(const char *label, const char *name, int level, int on);

static void tsp_ldo_on(int on)
{
	const char *reg_label = "WLAN1";
	const char *reg_name = "QT602240_ldo_3V3 LDO13";

	vreg_onoff(reg_label, reg_name, 3300, on);
...
//    DEBUG;
 ...
 //   qt602240->input_dev->name = "qt602240_ts_input";
	qt602240->input_dev->name = "quantum-touchscreen";
...
uint8_t mxt_unlock_bootloader(void)
{
    ...
    ret = boot_qt602240_i2c_write(0,buf,2);
    if(ret < 0) 
    {
...
void TSP_Restart(void)
{
	...
	mdelay(100);	// at least 40 msec recommended by manual Docu. 
            ...
            while(1) { 
                ...
                if(read_status_flag==1)     {
                    ...
                    if((boot_status & QT_WAITING_BOOTLOAD_COMMAND) == QT_WAITING_BOOTLOAD_COMMAND) {
                        if(mxt_unlock_bootloader() == WRITE_MEM_OK) 
			{
                            msleep(10);
