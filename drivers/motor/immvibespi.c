...
static int32_t ImmVibeSPI_ForceOut_AmpDisable(u_int8_t nActuatorIndex)
{
	if (g_bampenabled) 
	{
		...
		clk_disable(vib_clk);
		gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.vib_pwm_gpio, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
static int32_t ImmVibeSPI_ForceOut_AmpEnable(u_int8_t nActuatorIndex)
{
	if (!g_bampenabled) 
	{
		...
		gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.vib_pwm_gpio, 3, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
static int32_t ImmVibeSPI_ForceOut_Initialize(void)
{
	...
	*/
	...
	gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.vib_en_gpio,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (ret < 0) 
	{
		printk(KERN_ERR "[VIB] FAIL gpio_tlmm_config(%d)\n",
	...
	{
		ret = PTR_ERR(vreg_ldo19);
		pr_err("%s: wlan2 vreg get failed (%d)\n", __func__, ret);
		goto err2;
	}

	ret = regulator_set_voltage(vreg_ldo19,3000000,3000000);
	if (ret) 
	{
