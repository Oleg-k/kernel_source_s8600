...
int vibe_init(void)
{
	...
	if (gpio_tlmm_config(GPIO_CFG(GPIO_VIBTONE_EN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE))
		pr_err("%s: gpio_tlmm_config (gpio=%d) failed\n",
		       __func__, GPIO_VIBTONE_EN);

	if (gpio_tlmm_config(GPIO_CFG(GPIO_VIBTONE_PWM, 3, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE))
			pr_err("%s: gpio_tlmm_config (gpio=%d) failed\n",
			       __func__, GPIO_VIBTONE_PWM);

	if (gpio_is_valid(GPIO_VIBTONE_EN)) 
	{
		if (gpio_request(GPIO_VIBTONE_EN, "GPIO_VIBTONE_EN"))
