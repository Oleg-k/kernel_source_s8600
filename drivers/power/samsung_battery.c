...
const int temp_table[][2] =  {		//for S8600
	/* ADC, Temperature (C) */
	{ 1980,		-200},
	{ 1914,		-150},
	{ 1845,		-100},
...
#else
#if defined(CONFIG_MACH_PREVAIL2) || defined(CONFIG_MACH_ICON) || defined(CONFIG_MACH_S8600)
...
#define BATT_TEMP_LPM_HIGH_BLOCK   	550  // 47`C
#define BATT_TEMP_LPM_HIGH_RECOVER 	615  // 44`C
#define BATT_TEMP_LPM_LOW_BLOCK   	1718 // -3`C fixed 20110730  
#define BATT_TEMP_LPM_LOW_RECOVER  	1658 // 0`C
...
static void sec_bat_use_module(int module, int enable)
{
	...
	if (!enable && (msm_batt_info.batt_use == msm_batt_info.batt_use_wait)) 
	{
...
static void msm_batt_chg_en(chg_enable_type enable)
{
	...
	if (enable == START_CHARGING) 
	{
		...
		if ((chg_start_time_event == 0) && (msm_batt_info.charging_start_time == 0)) 
		{
...
static int msm_batt_check_full_charging(int chg_current_adc)
{
	...
	ktime_t	current_time;
	struct timespec ts;
	bool full_current_condition;

	current_time = alarm_get_elapsed_realtime();
	ts = ktime_to_timespec(current_time);

#if defined(CONFIG_MACH_ICON) || defined(CONFIG_MACH_PREVAIL2)
	full_current_condition = smb328a_check_bat_full();
	pr_info("%s: full_current_condition = %d\n",
		__func__, full_current_condition);
#endif

#if defined(CONFIG_MACH_S8600) 
	full_current_condition = max14577_check_bat_full();
	pr_info("%s: full_current_condition = %d\n", __func__, full_current_condition);
#endif
	...
	{
		charging_time = 0xFFFFFFFF - msm_batt_info.charging_start_time + ts.tv_sec;
	}

	msm_batt_info.charging_passed_time = charging_time;
	pr_info("%s: start_time = %u, chg_passed_time = %u\n", __func__, msm_batt_info.charging_start_time, msm_batt_info.charging_passed_time);
	...
	{
		if (full_current_condition == true) 
		{
			/* fully charged !*/
	...
	{
		pr_debug("[BATT] %s: voltage = %d(< %d)\n", __func__, msm_batt_info.battery_voltage, BATT_FULL_CHARGING_VOLTAGE);
	}
#else
	if (msm_batt_info.battery_voltage >= BATT_FULL_CHARGING_VOLTAGE) {
...
{
	if ((msm_batt_info.batt_full_check == 0) || (msm_batt_info.batt_recharging == 1) ||
...
static int msm_batt_check_level(int battery_level)
{
	...
	if (msm_batt_info.batt_full_check && !msm_batt_info.is_timeout_chgstop) 
	{
		battery_level = 100;
	} 
...
*/
	...
	if (msm_batt_info.check_full_state == true && msm_batt_info.batt_status == POWER_SUPPLY_STATUS_DISCHARGING
		&& !msm_batt_info.is_timeout_chgstop) 
	{
...
#define LOW_RECOVERY	3
	...
	if (msm_batt_info.charging_source != NO_CHG) 
	{
		 ...
		 else if ((temp_adc <= high_recover) && (temp_adc >= low_recover)) 
		{
			...
			if ((prev_health == POWER_SUPPLY_HEALTH_OVERHEAT) || (prev_health == POWER_SUPPLY_HEALTH_COLD))
				new_health = POWER_SUPPLY_HEALTH_GOOD;
		} 
		...
		if (prev_health != new_health) 
		{
			if (msm_batt_info.charging_source == NO_CHG) 
			{
				...
				pr_info("[BATT] %s: Health changed by temperature! (ADC = %d, %s-> %s)\n", __func__,
					temp_adc, health_text[prev_health], health_text[new_health]);
			...
			{	/* in charging */
				if (new_health != POWER_SUPPLY_HEALTH_GOOD) 
				{
					...
					pr_info("[BATT] %s: Block charging! (ADC = %d, %s-> %s)\n", __func__, temp_adc,
						health_text[prev_health], health_text[new_health]);
	...
	void *buf, void *data)
{
	...
	if (msm_batt_info.batt_api_version == BATTERY_RPC_VER_4_1) 
	{
		...
		data_ptr_4_1->batt_handle = be32_to_cpu(buf_ptr_4_1->batt_handle);
...
void msm_set_cable(struct msm_battery_callback *ptr, enum cable_type_t status)
{
...
static void msm_batt_cable_status_update(void)
{
	...
	if ((chg->cable_status == CABLE_TYPE_NONE) || (chg->cable_status == CABLE_TYPE_UNKNOWN) || (chg->cable_status == CABLE_TYPE_JIG)) 
	{
		msm_batt_info.charging_source = NO_CHG;
		msm_batt_info.batt_status = POWER_SUPPLY_STATUS_DISCHARGING;
	 ...
	 else  if ((chg->cable_status == CABLE_TYPE_CAR_DOCK) || (chg->cable_status == CABLE_TYPE_DESK_DOCK) || (chg->cable_status == CABLE_TYPE_UART)) 
	{
		pr_debug("%s: ovp_en=%d\n", __func__, ovp_en);
		if (max14577_vbus_valid()) 
		{
			if (msm_batt_info.batt_health != POWER_SUPPLY_HEALTH_UNSPEC_FAILURE)
				msm_batt_info.batt_health = POWER_SUPPLY_HEALTH_GOOD;
	 ...
	 else 
	{
		...
		if (chg->cable_status == CABLE_TYPE_USB || chg->cable_status == CABLE_TYPE_CDP) 
		{
	...
	if ((msm_batt_info.prev_cable != msm_batt_info.cable_status) ||
		(msm_batt_info.pre_batt_status != msm_batt_info.batt_status)) 
	{
		queue_delayed_work(msm_batt_info.msm_batt_wq, &msm_batt_info.msm_batt_work, 0);
		pr_info("%s: status changed, work queue called\n", __func__);
	}

	msm_batt_info.prev_cable = msm_batt_info.cable_status;

	if (likely(!(charging_boot && msm_batt_info.batt_status != POWER_SUPPLY_STATUS_CHARGING)))
...
void msm_batt_cable_status_update_ext(int vbus_in)
{
	...
	if ((chg->cable_status == CABLE_TYPE_USB) || (chg->cable_status == CABLE_TYPE_TA) || (chg->cable_status == CABLE_TYPE_CDP))
...
static int __devinit msm_batt_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK_DEFERRABLE(&msm_batt_info.msm_batt_work, msm_batt_check_event);
	queue_delayed_work(msm_batt_info.msm_batt_wq, &msm_batt_info.msm_batt_work, 0);
