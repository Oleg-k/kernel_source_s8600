...
//kk0704.park TEMP :: #define WORKAROUND_FOR_LOW_SPEED_I2C
#endif
...
#define CAM_FLASH_ENSET 		164
#define CAM_FLASH_FLEN 			165

#define FULL_FLASH 20
#define PRE_FLASH 7


#if defined(CONFIG_MACH_ANCORA) || defined(CONFIG_S8600) || defined(CONFIG_MACH_HUANGHE)
...
#define CAM_MEGA_nRST		174
#define CAM_MEGA_STBY		175
#define CAM_VT_nSTBY		31
#define CAM_VT_nRST		132
...
static uint32_t sensor_started = CAMERA_NOT_START;

static DECLARE_WAIT_QUEUE_HEAD(s5k4ecgx_wait_queue);
DECLARE_MUTEX(s5k4ecgx_sem);

static int camera_power_status = 2;
/*=============================================================
...
static int s5k4ecgx_check_vsync(); // add for ESD

#ifdef CONFIG_LOAD_FILE
    static int s5k4ecgx_regs_table_write(char *name);
    #define BURST_MODE_BUFFER_MAX_SIZE 2700		// XXXom
    unsigned char s5k4ecgx_buf_for_burstmode[BURST_MODE_BUFFER_MAX_SIZE]; // XXXom

#endif
bool isRecording = false; //sjlee_0207 (to check recording status)
EXPORT_SYMBOL(isRecording);

#define CAM_GPIO_PM_TO_SYS(pm_gpio)     (pm_gpio + NR_GPIO_IRQS)

int cam_gpio_ctl(int gpio, bool on)
{
	int sys_gpio, err;

	sys_gpio=CAM_GPIO_PM_TO_SYS(gpio);

	err = gpio_request(sys_gpio, NULL);
	if (err) 
	{
...
void s5k4ecgx_set_fps(char value)
{
    switch(value)
    {
        ...
        case EXT_CFG_FRAME_FIX_20 :
            ...
            mdelay(100); /*delay for pclk stabilization (noframe issue) 2011-01-20 */
        break;
...
static int s5k4ecgx_check_vsync()
{
    printk("%s, start \n","S5K4ECGX" );
    ...
    s5k4ecgx_sensor_write(0x002C, 0x7000);
    s5k4ecgx_sensor_write(0x002E, 0x01A8);
    s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&Val);

    printk("%s, 1st Val is 0x%x\n","S5K4ECGX", Val);
	
    if(Val != 0x0A0A)		
    	{
   		s5k4ecgx_set_power(false);
		s5k4ecgx_set_power(true);	
		//s5k4ecgx_set_preview(); Chang to this func.
    	}

    s5k4ecgx_sensor_write(0x002C, 0xD000);
    s5k4ecgx_sensor_write(0x002E, 0x0060);
    s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&Val);

    printk("%s, 2nd Val is 0x%x\n","S5K4ECGX", Val);	
...
//    if(s5k4ecgx_status.flash_mode == EXT_CFG_FLASH_OFF)return 0;
    ...
    /* initailize falsh IC */
	cam_gpio_ctl(CAM_FLASH_FLEN, false);
	cam_gpio_ctl(CAM_FLASH_ENSET, false);
	...
	cam_gpio_ctl(CAM_FLASH_FLEN, true);
    ...
    else if(lux_val == MACRO_FLASH)
    {
        ...
        {
	        udelay(1);
		cam_gpio_ctl(CAM_FLASH_ENSET,  true);
	        udelay(1);
		cam_gpio_ctl(CAM_FLASH_ENSET,  false);
        }
		cam_gpio_ctl(CAM_FLASH_ENSET, true);
...
#define Mapping_H	1	//(640/640)
#define Mapping_V	1	//(480/480)
#define sensor_display_H		640
#define sensor_display_V		480

void s5k4ecgx_set_touch_pos(uint32_t posX, uint32_t posY)
{
	uint32_t inner_window_start_X=0, inner_window_start_Y=0, inner_window_width=0, inner_window_height=0;
	uint32_t outer_window_start_X=0, outer_window_start_Y=0, outer_window_width=0, outer_window_height=0;
	unsigned short ScndWinStartX=0, FstWinStartX=0, ScndWinStartY=0, FstWinStartY=0;

	if(posX==-1 && posY==-1) {
		S5K4ECGX_WRITE_LIST(s5k4ecgx_AF_Pos_Reset);
		mdelay(100);
		return 0;
	}//somin.kim add for AF pos default setting

	int mapped_X = posX * Mapping_H;
	int mapped_Y = posY * Mapping_V;

	printk("[S5K4ECGX]touch(X,Y): [%d,%d]\n", posX, posY);
	
	s5k4ecgx_sensor_write(0x002C, 0x7000);
	s5k4ecgx_sensor_write(0x002E, 0x02A0);
	s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&inner_window_width);
	inner_window_width = (inner_window_width * sensor_display_H ) / 1024;

	s5k4ecgx_sensor_write(0x002C, 0x7000);
	s5k4ecgx_sensor_write(0x002E, 0x02A2);
	s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&inner_window_height);
	inner_window_height = (inner_window_height * sensor_display_V ) / 1024;

	s5k4ecgx_sensor_write(0x002C, 0x7000);
	s5k4ecgx_sensor_write(0x002E, 0x0298);
	s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&outer_window_width);
	outer_window_width = (outer_window_width * sensor_display_H ) / 1024;

	s5k4ecgx_sensor_write(0x002C, 0x7000);
	s5k4ecgx_sensor_write(0x002E, 0x029A);
	s5k4ecgx_sensor_read(0x0F12, (unsigned short*)&outer_window_height);
	outer_window_height = (outer_window_height * sensor_display_V ) / 1024;

	printk("[S5K4ECGX]inner_window_size(X,Y): [%d,%d]\n", inner_window_width, inner_window_height);
	printk("[S5K4ECGX]outer_window_size(X,Y): [%d,%d]\n", outer_window_width, outer_window_height);
	...
	else if(mapped_Y >= ((sensor_display_V-1)-outer_window_height/2))
	{
		inner_window_start_Y = mapped_Y - inner_window_height/2;
		outer_window_start_Y = (sensor_display_V-1) - outer_window_height;
	}
	...
	printk("[S5K4ECGX]inner_window_start(X,Y): [%d,%d]\n", inner_window_start_X, inner_window_start_Y);
	printk("[S5K4ECGX]outer_window_start(X,Y): [%d,%d]\n", outer_window_start_X, outer_window_start_Y);
		
	ScndWinStartX = inner_window_start_X * 1024 / sensor_display_H;
	FstWinStartX = outer_window_start_X * 1024 / sensor_display_H;
	ScndWinStartY = inner_window_start_Y * 1024 / sensor_display_V;
	FstWinStartY = outer_window_start_Y * 1024 / sensor_display_V;
	
	s5k4ecgx_sensor_write(0x0028, 0x7000);
	s5k4ecgx_sensor_write(0x002A, 0x029C);
	s5k4ecgx_sensor_write(0x0F12, ScndWinStartX);

	s5k4ecgx_sensor_write(0x0028, 0x7000);
	s5k4ecgx_sensor_write(0x002A, 0x0294);
	s5k4ecgx_sensor_write(0x0F12, FstWinStartX);

	s5k4ecgx_sensor_write(0x0028, 0x7000);
	s5k4ecgx_sensor_write(0x002A, 0x029E);
	s5k4ecgx_sensor_write(0x0F12, ScndWinStartY);

	s5k4ecgx_sensor_write(0x0028, 0x7000);
	s5k4ecgx_sensor_write(0x002A, 0x0296);
	s5k4ecgx_sensor_write(0x0F12, FstWinStartY);

	s5k4ecgx_sensor_write(0x0028, 0x7000);
	s5k4ecgx_sensor_write(0x002A, 0x02A4);
	s5k4ecgx_sensor_write(0x0F12, 0x0001);

	printk("[S5K4ECGX]FstWinStar(X,Y): [%d,%d]\n", FstWinStartX, FstWinStartY);
	printk("[S5K4ECGX]ScndWinStar(X,Y): [%d,%d]\n", ScndWinStartX, ScndWinStartY);
	s5k4ecgx_status.touch_af = 1;
...
int s5k4ecgx_set_af(char value)
{
    ...
    switch(value)
    {
        ...
        case EXT_CFG_AF_SET_AE_FOR_FLASH :
            CAMDRV_DEBUG("%s : EXT_CFG_AF_SET_AE_FOR_FLASH value : %d\n", __func__,s5k4ecgx_status.flash_mode);
            s5k4ecgx_get_lux(&s5k4ecgx_status.current_lux);
//		s5k4ecgx_status.flash_mode = 0;
            if(s5k4ecgx_status.flash_mode != EXT_CFG_FLASH_OFF)
            {
                if(s5k4ecgx_status.flash_mode == EXT_CFG_FLASH_AUTO)
                {
                    if(s5k4ecgx_status.current_lux > 0x0032)break; //somin.kim 0111 change flash condition
                }
                s5k4ecgx_sensor_write(0x0028, 0x7000);
...
//            S5K4ECGX_WRITE_LIST(s5k4ecgx_ISO_50); remove somin.kim 0203s
        break;
...
void s5k4ecgx_set_preview_size(int size)
{
    ...
    //s5k4ecgx_set_fps(s5k4ecgx_status.fps); somin.kim 0401 remove for duplication.
...
void s5k4ecgx_set_preview(void)
{
    ...
    if(s5k4ecgx_status.camera_initailized == false)
    {
	 ...
	 if(rc < 0) sensor_started = CAMERA_START_FAIL;
	 else sensor_started = CAMERA_START_SUCCESS;
        mdelay(100);
#endif
...
//somin.kim reduce delay 0119    if(s5k4ecgx_status.camera_initailized == false) mdelay(100);  /*delay for pclk stabilization (noframe issue) 2011-01-20 */
    ...
    s5k4ecgx_status.touch_af=0;
    if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT ||s5k4ecgx_status.scene == EXT_CFG_SCENE_FIREWORK)
    {
//        S5K4ECGX_WRITE_LIST(s5k4ecgx_Scene_Nightshot); // remove somin.kim 0203
    ...
    {
//somin.kim reduce delay 0119	mdelay(100);
	s5k4ecgx_status.camera_on = true;
    }
    isRecording = false; //sjlee_0207 (to check recording status)
}
...
void s5k4ecgx_set_capture(void)
{
    ...
    if(s5k4ecgx_status.flash_mode != EXT_CFG_FLASH_ON && s5k4ecgx_status.current_lux > 0x0032) //somin.kim 0111 change flash condition
    {
...
//    CAMDRV_DEBUG("s5k4ecgx_sensor_ext_config %d %d %d \n",cfg_data.cmd, cfg_data.value_1,cfg_data.value_2);
	...
	case EXT_CFG_SET_TOUCHAF_POS:
		s5k4ecgx_set_touch_pos(cfg_data.value_1, cfg_data.value_2);
	break;
...
*/
		...
		if(s5k4ecgx_status.camera_mode == 1)
			{
				 s5k4ecgx_set_brightness(s5k4ecgx_status.brightness); // smkim add set brightness 
			}
        break;
        case EXT_CFG_ESD_FRAME:
		s5k4ecgx_check_vsync();	
        break;		
        case EXT_CFG_SET_RECORDING_MODE: //sjlee_0207 (to check recording status)
            isRecording = cfg_data.value_1;
        break;
	case EXT_CFG_GET_SENSOR_STATUS:
              CAMDRV_DEBUG("EXT_CFG_GET_SENSOR_STATUS %d \n", sensor_started);
		cfg_data.value_1 = sensor_started;
	break;
	case EXT_CFG_RESET:
              CAMDRV_DEBUG("EXT_CFG_RESET\n");
//		s5k4ecgx_ctrl->settings.camera_initialized = false;
		s5k4ecgx_set_power(false);
		mdelay(50);
		s5k4ecgx_set_power(true);
		mdelay(50);
...
{
	int err;
	//0309 move gpio setting to board-huanghe.c file.
	if((camera_power_status != status ) && (status == 1)) //POWER ON
    {
		camera_power_status = status;
		...
		gpio_set_value(CAM_MEGA_nRST, 0);		
		gpio_set_value(CAM_VT_nRST, 0);
		gpio_set_value(CAM_VT_nSTBY, 0);

        /* initailize flash IC */
		mdelay(1);
	/* Enable V_VGA_D_1V8 */
        	gpio_set_value(CAM_EN5, 1);
	        mdelay(20);


	/* Enable V_CAM_C_1V2 - Core 1.2v */
        	gpio_set_value(CAM_EN1, 1);
	        mdelay(20);


        /* Enable V_CAM_A_2V8 - LDO2 2.8v */
	        gpio_set_value(CAM_EN2, 1);
	        mdelay(20);


        /* Enable V_CAM_IO_1V8 - LDO3 1.8v */
	        gpio_set_value(CAM_EN3, 1);
        	mdelay(20);


        /* Enable V_CAM_AF_2V8 - LDO4 2.8v */
	        gpio_set_value(CAM_EN4, 1);
        	mdelay(20);

		gpio_set_value(CAM_VT_nSTBY, 1);
		mdelay(1);

		msm_camio_sensor_clk_on(Pdev);
		mdelay(1);
...
//		msm_camio_clk_rate_set(24000000);
		...
		mdelay(1);
		gpio_set_value(CAM_MEGA_nRST, 1);
		mdelay(30);
...
//		mdelay(1);
	...
	else if ((camera_power_status != status ) && (status == 0) ) //POWER OFF
    {
	camera_power_status = status;
	...
	gpio_set_value(CAM_MEGA_nRST, 0);
	mdelay(1);
	msm_camio_sensor_clk_off(Pdev);
	gpio_set_value(CAM_MEGA_STBY, 0);
	gpio_set_value(CAM_VT_nRST, 0);

        /* Disable V_VGA_D_1V8 */
        gpio_set_value(CAM_EN5, 0);
        mdelay(20);

        //Disable V_CAM_AF_2V8 - LDO4 2.8v
        gpio_set_value(CAM_EN4, 0);
        mdelay(10);

        //Disable V_CAM_IO_1V8 - LDO3 1.8v
        gpio_set_value(CAM_EN3, 0);
        mdelay(10);

        //Disable V_CAM_A_2V8 - LDO2 2.8v
        gpio_set_value(CAM_EN2, 0);
        mdelay(10);

        /* Disable V_CAM_C_1V2 - Core 1.2v */
        gpio_set_value(CAM_EN1, 0);
        mdelay(10);
	...
	{
		printk("Camera Power status is already : %d ", camera_power_status);
	}
}
...
void s5k4ecgx_regs_table_init(void)
{
    ...
    CAMDRV_DEBUG("read file start");
    file_size = filp->f_path.dentry->d_inode->i_size;	
    l = 2000;
    CAMDRV_DEBUG("file_size = %d, l = %ld",file_size, l);

    dp = kmalloc(l, GFP_KERNEL);	
    if (dp == NULL) {
    	CAMDRV_DEBUG("Out of Memory");
    ...
    while(1) {
    	pos_copy = pos;
    	CAMDRV_DEBUG("pos = %d, pos_copy =  %d",pos,pos_copy);
    ...
    s5k4ecgx_regs_table = tunning_data;
    s5k4ecgx_regs_table_size = file_size;
...
//	For confirm copy reg file   //
//	printk("<<<<< COPYED DATA  >>>>>\n");
//	for(i=260000;i<269175;i++)
//		printk("%c",s5k4ecgx_regs_table[i]);
}
...
// add for tick noise 0401 reduce delay
            S5K4ECGX_WRITE_LIST(s5k4ecgx_AF_Normal_mode_1);
            if(s5k4ecgx_status.scene == EXT_CFG_SCENE_NIGHTSHOT)mdelay(250);
            else mdelay(100);
            S5K4ECGX_WRITE_LIST(s5k4ecgx_AF_Normal_mode_2);
...
//kk0704.park TEMP :: 	s5k4ecgx_set_power(true);
    /* Input MCLK = 24MHz */
//kk0704.park TEMP ::         msm_camio_clk_rate_set(24000000);
//kk0704.park TEMP :: 	s5k4ecgx_probe_init_sensor();
	//s->s_power = s5k4ecgx_set_power;
    s->s_init = s5k4ecgx_sensor_init;
    s->s_release = s5k4ecgx_sensor_release;
    s->s_config  = s5k4ecgx_sensor_config;
    s->s_camera_type = BACK_CAMERA_2D;
    s->s_mount_angle = 0;
//kk0704.park TEMP :: 	s5k4ecgx_set_power(false);
