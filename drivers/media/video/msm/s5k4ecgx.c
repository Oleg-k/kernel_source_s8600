...
#if defined(CONFIG_MACH_ANCORA) || defined(CONFIG_MACH_APACHE) || defined(CONFIG_MACH_S8600)
...
void s5k4ecgx_set_power(int status)
{
    ...
    if(status == 1) //POWER ON
    {
        ...
        gpio_set_value(CAM_5M_nRST, 0); // CAM_MEGA_nRST
        gpio_set_value(CAM_5M_nEN, 0); // CAM_MERA_STBY - disabled
        gpio_set_value(CAM_VGA_EN, 0);  // CAM_VT_nSTBY - disabled
        gpio_set_value(CAM_VGA_nRST, 0); // CAM_VT_nRST
        ...
        /* disable CAM_VT_nRST */
        gpio_set_value(CAM_VGA_nRST, 1);
        mdelay(20);

        /* Disable VGA CAM  */
        gpio_set_value(CAM_VGA_EN, 0);  // CAM_VT go to HI-Z. -> going to standby mode
	mdelay(5);

        gpio_set_value(CAM_5M_nEN, 1); //5M CAM ENABLE
        udelay(20);
        gpio_set_value(CAM_5M_nRST, 1); // CAM_MEGA_nRST ENABLE
        udelay(70);
    ...
    else //POWER OFF
    {
        ...
        gpio_set_value(CAM_VGA_EN, 0);  // CAM_VT_nSTBY
        gpio_set_value(CAM_VGA_nRST, 0); // CAM_VT_nRST
       
        gpio_set_value(CAM_5M_nRST, 0);      //nRST
        udelay(50);
        ...
        gpio_set_value(CAM_5M_nEN, 0);     //Disable 5M camera
...
static int s5k4ecgx_probe_init_sensor()
{
    ...
    gpio_set_value(CAM_5M_nEN, 1);        // standby
    msleep(15);
    gpio_set_value(CAM_5M_nRST, 1);      // reset
    msleep(50);
#endif
...
#elif defined(CONFIG_MACH_ANCORA) || defined(CONFIG_MACH_S8600)
    if(s5k4ecgx_status.id == 0x0011)
