...
static int sr130pc10_set_power(int onoff)
{
  ...
  if(onoff)
  {
    ...
    gpio_set_value(CAM_5M_nRST, 0);	
    gpio_set_value(CAM_5M_nEN, 0);	

    gpio_set_value(CAM_VGA_nRST, 0);	
    gpio_set_value(CAM_VGA_EN, 0);	//CAM_VGA_nSTB


    //LDO2 2.8v V_CAM_A_2V8
    gpio_set_value(CAM_EN2, 1);	
    mdelay(20);        

    //LDO3 1.8v V_CAM_IO_1V8 - powers both VDDIO 1.8V and 1.3M Core 1.8V       
    gpio_set_value(CAM_EN3, 1);	
    mdelay(20);

    //LDO5 1V8 V_VGA_D_1V8
    gpio_set_value(CAM_EN5, 1);	
    mdelay(100);

    gpio_set_value(CAM_VGA_EN, 1); //CAM_VGA_EN UP - enable vga camera
    udelay(5);
  ...
  else  //Disable VGA camera
  {
    gpio_set_value(CAM_5M_nRST, 0); //REST -> DOWN
    gpio_set_value(CAM_5M_nEN, 0);

    mdelay(7);
    gpio_set_value(CAM_VGA_nRST, 0); //REST -> DOWN
    mdelay(2);
    ...
    gpio_set_value(CAM_VGA_EN, 0); //STBY -> DOWN
    udelay(5);
