...
#if defined(CONFIG_MACH_ANCORA) || defined(CONFIG_MACH_HUANGHE) || defined(CONFIG_MACH_S8600)
#define CAM_FLASH_FLEN 4
#define CAM_FLASH_ENSET 5
#else
...
static ssize_t ledflash_read(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	printk("[HCHYUN] %s\n", __func__);
	/* initailize falsh IC */
	cam_pmic_gpio_ctl(CAM_FLASH_ENSET,0); 
	cam_pmic_gpio_ctl(CAM_FLASH_FLEN,0);
	...
	{
		udelay(1);
		cam_pmic_gpio_ctl(CAM_FLASH_ENSET,1);
		udelay(1);
		cam_pmic_gpio_ctl(CAM_FLASH_ENSET,0);
	}
	cam_pmic_gpio_ctl(CAM_FLASH_ENSET,1); //value set
