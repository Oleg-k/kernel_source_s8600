...
void D4Hp3_SetAmpGain(unsigned char Att, int device)
{
	UINT8 abWritePrm = 0;
	unsigned char buf;

	printk("D4Hp3_SetAmpGain att = 0x%02x\n", Att);
	
	if(device == HEADSET)	{
		abWritePrm = (Att << (D4HP3_HPATT & 0xFF));
		D4Hp3_WriteRegisterByte(0x86, abWritePrm);
	}
	else if(device == SPEAKER)	{
		abWritePrm = (Att << (D4HP3_SPATT & 0xFF));
		D4Hp3_WriteRegisterByte(0x85, abWritePrm);
	}
	else	{
		printk("wrong argument.\n");
	}
}
...
#if defined(CONFIG_MACH_HUANGHE)
...
void yda165_speaker_headset_onoff(int onoff) /* speaker+headset path amp onoff */
{
	...
	if (onoff)
	{
		...
		cur_mode = 2;
		pr_info(MODULE_NAME ":speaker+headset on,cur_mode=%d",cur_mode);		
		/* input */
		stInfo.bLine1Gain = g_ampgain[cur_mode].in1_gain;		/* LINE1 Gain Amp */
		stInfo.bLine2Gain = g_ampgain[cur_mode].in2_gain;		/* LINE2 Gain Amp */
#if defined(CONFIG_MACH_HUANGHE)
		...
		stInfo.bHpZcs = 0;;				/* HP zero cross mute setting, on(0) / off(1) */
		stInfo.bHpCh = 0;
#if defined(CONFIG_MACH_HUANGHE)				/* HP channel, stereo(0)/mono(1) */
		stInfo.bHpMixer_Line1 = 0;		/* HP mixer LINE1 setting */
...
void yda165_headset_fm_onoff(int onoff) /* headset path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
	
	if (onoff)
	{
		pr_info(MODULE_NAME ":fm headset on\n");
		cur_mode = 3;
	...
	{
		pr_info(MODULE_NAME ":fm headset off\n");
		D4Hp3_PowerOff();
	}
}

void yda165_speaker_fm_onoff(int onoff) /* fm speaker path amp onoff */
{
	D4HP3_SETTING_INFO stInfo;
#if AMPREG_DEBUG	
	unsigned char buf;
	...
	if (onoff)
	{
		pr_info(MODULE_NAME ":fm speaker on\n");
		cur_mode = 3;
	...
	{
		pr_info(MODULE_NAME ":fm speaker off\n");
		D4Hp3_PowerOff();
	}
}
